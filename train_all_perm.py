...
def main(args):
    ...
    for reactions in trainloader:
        ...
        for i, n in enumerate(num_reactants):
            lengths.append(math.factorial(n)*n)
        ...
        b_loss = torch.stack([x.view(-1, n).sum(1).mul(-1).logsumexp(0).mul(-1) for n, x in zip(num_reactants, b_loss)])
        b_corrects = torch.stack([x.view(-1, n).all(1).any(0) for n, x in zip(num_reactants, b_corrects)])
